...
class GaussianDiffusion:
    ...
    def training_losses(self, model,  x_start, t, classifier=None, model_kwargs=None, noise=None, labels=None,
        ...
        # ★★★★★★★★★★★★★★★★★★★ 最終修正箇所 ★★★★★★★★★★★★★★★★★★★
        # =================================================================================
        # 新しいデータ形式に合わせて、ターゲットと条件を正しく定義します。
        if mode == 'i2i':
            # x_start はターゲット画像 (T1c) そのもの
            target = x_start
            # model_kwargs から条件画像 (T1n) を取り出す
            cond_1 = model_kwargs['cond_1']
        ...
        noise = th.randn_like(target)  # ノイズをサンプリング
        LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH = dwt(noise)
        noise_dwt = th.cat([LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH], dim=1)
        x_t = self.q_sample(x_start_dwt, t, noise=noise_dwt)  # ノイズ付与

        if mode == 'i2i':
            x_t = th.cat([x_t, cond_dwt], dim=1)  # 条件をチャンネル次元で連結
