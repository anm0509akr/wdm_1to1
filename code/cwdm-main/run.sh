...
MODE='sample';             # train, sample, auto (for automatic missing contrast generation)
DATASET='brats';          # brats
MODEL='unet';             # 'unet'
...
ITERATIONS=1200;          # training iteration (as a multiple of 1k) checkpoint to use for sampling 1200
...
if [[ $MODEL == 'unet' ]]; then
  ...
  BATCH_SIZE=1;
  IMAGE_SIZE=224;
  IN_CHANNELS=16;           # Change to work with different number of conditioning images 8 + 8x (with x number of conditioning images)
  NOISE_SCHED='linear';
else
