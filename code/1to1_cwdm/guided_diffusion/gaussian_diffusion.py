...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        # 条件付き生成の場合、条件画像を連結してモデルへの入力を作成
        if model_kwargs:
            # x_t (ノイズ付きターゲット) と cond (条件) を連結
            model_input = th.cat([x_t, model_kwargs['cond_1']], dim=1)
        else:
            model_input = x_t
            
        # モデルに渡してノイズを予測
        model_output = model(model_input, self._scale_timesteps(t))

        # 損失計算のターゲットは、追加したノイズそのもの
        target = noise

        # 損失計算
        terms = {}
        # ... (この部分は元の論文の損失計算ロジックに依存します) ...
        # このリポジトリの元々の損失計算方法（L2損失）
        terms["loss"] = mean_flat((target - model_output) ** 2)
        terms["pred_xstart"] = self.predict_start_from_noise(x_t, t, model_output)
